...
def get_id_vedio():
    ...
    try:
        ...
        print(f'[{Fore.LIGHTMAGENTA_EX}${Fore.RESET}] ﺓﺪﻫﺎﺸﻣ ﺖﻤﺗ')
        time.sleep(2)
    except:
        print(f'[{Fore.LIGHTMAGENTA_EX}${Fore.RESET}] ﻞﺸﻓ')
        input()
    def view():
...
def get_id_vedio():
    ...
    try:
        ...
        id_post1 = re.findall('"video":{"id":"(.*?)"', req)
        idpost = "".join(id_post1)
        print(f'[{Fore.LIGHTMAGENTA_EX}${Fore.RESET}] ﺓﺪﻫﺎﺸﻣ ﺖﻤﺗ مقطع')
        time.sleep(2)
    except:
        print(f'[{Fore.LIGHTMAGENTA_EX}${Fore.RESET}] رابط خطأ')
        input()
    def view():
        url = 'https://api16-core-c-alisg.tiktokv.com/aweme/v1/aweme/stats/?ac=WIFI&op_region=SA'
        headers = {
